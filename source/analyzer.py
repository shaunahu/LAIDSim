...
class PreferenceAnalyzer:
    ...
    def plot_alteration_degee(self, id_scenario: int = 0, id_run: int = 0):
        df = self.db.read_dataframe("environment_result")
        df = df.loc[(df["id_scenario"] == id_scenario) & (df["id_run"] == id_run)]
        values_dict = {
            "Alteration Degree": df["alteration_degree"]
        }
        figure = plt.figure(figsize=(12, 6))
        ax = figure.add_axes((0.1, 0.1, 0.8, 0.8))
        # ax.set_ylim(0, 4039)
        ...
        for key, values in values_dict.items():
            ...
            bottom += values
        ax.legend(fontsize=12)
        self.save_fig(figure, f"InformationAlteration_S{id_scenario}R{id_run}")
